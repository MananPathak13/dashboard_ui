...
type DeleteActionKeys = keyof typeof deleteActionMap;
...
const forms: {
  [key: string]: (setOpen: React.Dispatch<React.SetStateAction<boolean>>,
    type: "create" | "update",
    data?: any,
    relatedData?: any
  ) => JSX.Element;
...
const FormModal = ({
  ...
  const Form = () => {
    ...
    const [state, formAction] = useActionState(deleteActionMap[table as DeleteActionKeys], {
      success: false,
      error: false,
    })
  ...
  return (
    <>
      ...
      {open && (
        <div className="w-screen h-screen absolute left-0 height-0 top-0 bg-black bg-opacity-60 z-50 flex items-center justify-center">
          <div className="bg-white p-4 rounded-md relative w-[90%] md:w-[70%] lg:w-[60%] xl:w-[50%] 2xl:w-[40%]">
            <Form />
            <div className="absolute top-4 right-4 cursor-pointer"><Image src="/close.png" alt="" width={14} height={14} onClick={() => setOpen(false)} /></div>
          </div>
